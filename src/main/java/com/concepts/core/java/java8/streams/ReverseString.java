...
public class ReverseString {
	...
	public static String strRev(String string) {
		...
		// 2nd method
		Iterator<Character> revIter = string.chars().mapToObj(item -> new Character((char) item))
				.collect(Collectors.toCollection(ArrayDeque::new)).descendingIterator();

		reverseStr = StreamSupport.stream(Spliterators.spliteratorUnknownSize(revIter, Spliterator.ORDERED), false)
				.map(Object::toString).collect(Collectors.joining(""));

		String[] arr = { "earth", "mars", "mercury" };
		List<String> ll = Arrays.stream(arr).map(s -> new StringBuilder(s).reverse().toString())
